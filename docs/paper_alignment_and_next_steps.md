...
| OpenAI-compatible LLMPlanner | `meteo_agent_da/agent/planner.py` | ToolBench 的工具选择与 solution path annotation | 支持 `--planner llm`，让模型真实生成工具调用计划 |
| 配置外置化与 env 覆盖 | `configs/default.yaml`、`ProjectConfig.from_file()` | ScienceAgentBench 的可复现设置要求 | 减少硬编码路径，方便跨机器复现 |
| 受控命令执行开关 | `pasnet_runner.py`、`CommandExecutor` | ScienceAgentBench 的 execution result 评估 | 三重开关后才执行 PASNet 命令，默认安全 dry-run |
...
| report 级 verifier | `meteo_agent_da/bench/verifiers.py` | ScienceAgentBench 的 success criteria / verifier | 检查 path、command、metric、artifact 是否可信 |
| 多 report 评估脚本 | `meteo_agent_da/bench/evaluate_trace.py` | ScienceAgentBench 的多次运行和聚合评估思路 | 可以直接比较 text-only/tool-agent/SFT/DPO |
| trace 级 SFT 数据 | `meteo_agent_da/post_training/build_sft_data.py` | AgentTuning 的交互轨迹 SFT | 从 `trace.jsonl` 保留 plan、tool、observation、report |
| DPO-style preference 数据 | `meteo_agent_da/post_training/build_preference_data.py` | AgentTuning 的 trajectory filtering；DPO 类偏好数据流程 | 将 chosen/rejected report 转成偏好学习样本，并拒绝 error chosen |
...
4. ToolEval 风格评估
   ...
   - 当前已新增 `tool_recall`、`tool_precision`、`tool_success_rate`、`verifier_pass_rate`、`pass_rate`。
...
2. 可执行 verifier
   - 当前已新增 `bench/verifiers.py` 的 report 级 verifier：
   ...
   - 后续还需要补 cost verifier：工具步数、执行时间、API 费用。
...
2. SFT 数据升级
   - 当前 `post_training/build_sft_data.py` 已读取同目录 `trace.jsonl`，保留：
     - planner 输出；
     - 每一步 tool call 的参数；
     - tool observation；
     - reflector 修正；
     - final report。
   - 后续需要只保留 verifier/pass_rate 通过的成功轨迹进入训练集。
...
4. DPO 数据
   - 当前已新增 `build_preference_data.py`，支持 chosen/rejected report，并默认拒绝 `status=error` 的 chosen。
   - 后续需要自动构造 rejected：
     - text-only 无工具证据；
     - 漏掉关键工具；
     - 命令参数错误；
...
2. Tool execution gate
   ...
   - 当前 `ToolRegistry.run()` 已有 risky 检查，`pasnet_runner` 也只有在 `--execute --allow-risky --run-commands` 同时打开时才会执行命令。
   - 但还缺：
     - 参数 schema 校验；
     - 重复调用拦截；
     - 敏感路径/环境变量脱敏；
...
2. 新增 `bench/run_pasbench.py`，支持批量跑 `text_only|heuristic_tool|llm_tool` 和 `--attempts 3`。
3. 给 `LLMPlanner` 增加本地 vLLM/OpenAI-compatible 多后端配置和重试机制。
4. 用 verifier/pass_rate 过滤成功 tool-agent 轨迹，形成 SFT 训练集。
5. 自动构造 DPO rejected，包括 text-only、漏工具、错误命令、指标幻觉和 artifact 缺失。
6. 做第一张核心结果表：
